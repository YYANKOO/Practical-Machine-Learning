...
###### In data modeling, the training data is divided into training dataset and validation dataset. The "classe" variable in the training set is the outcome. The validation data is used to validate the model with the expected out-of-sample error rate of less than 1%, or 99% accuracy. The results show that the training model which is build using Random Forest is able to meet the 99% accuracy. Also, it predicts the 20 test cases in the testing data with 100% accuracy.
...
## Data Modeling
###### Random Forests, Decision Tree and Generalized Boosted Model are used for model prediction. The one with highest accuracy will be used to model the testing data.
#### Random Forest
